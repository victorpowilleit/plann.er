...
export function App() {
  ...
  const [emailsToInvite, setEmailsToInvite] = useState<string[]>(['victor.powilleit@gmail.com', 'outro']);
        ...
        <div className="space-y-4">
          ...
          <div className="h-16 bg-zinc-900 px-4 rounded-xl flex items-center shadow-shape gap-3">
            <div className="flex items-center gap-2 flex-1">
              ...
              <input disabled={isGuestInputOpen} type="text" placeholder="Para onde você vai?"
                     className="bg-transparent text-lg placeholder-zinc-400 outline-none flex-1"/>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="size-5 text-zinc-400"/>
              <input disabled={isGuestInputOpen} type="text" placeholder="Quando?"
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center">
          <div className="w-[640px] rounded-xl py-5 px-6 shadow-shape bg-zinc-900 space-y-5">
            ...
            <p className="text-sm text-zinc-400">
              Os convidados irão receber e-mails para confirmar a paryicipação na viagem.
            </p>

            </div>
            <div className="flex flex-wrap gap-2">

              {emailsToInvite.map(mail => (
                <div key={mail} className="flex py-1.5 px-2.5 rounded-md bg-zinc-800 items-center gap-2">
                <span className="text-zinc-300 lowercase">
                  {mail}
                </span>
                  <button onClick={()=>removeEmailFromInvites(mail)} type="button">
            ...
            <form onSubmit={e=>addNewEmailToInvite(e)} className="p-2.5 bg-zinc-950 border-zinc-800 rounded-lg flex items-center gap-2">
